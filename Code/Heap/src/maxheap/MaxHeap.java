...
public class MaxHeap<E> {
    ...
    public MaxHeap(E[] array, Comparator<? super E> comparator) {
        this.array = array;
    ...
    private void down(int parent) {
        ...
        while (parent * 2 + 1 < size) {
            ...
            if (cpr == null) {
                ...
                if (leftValue.compareTo(array[max]) > 0)
                    max = left;
                Objects.requireNonNull(array[max]);
                Comparable<? super E> maxValue = (Comparable<? super E>) array[max];
                if (right < size && maxValue.compareTo(array[right]) < 0)
                    max = right;
            } else {
                if (cpr.compare(array[left], array[max]) > 0)
                    max = left;
                if (right < size && cpr.compare(array[right], array[max]) > 0)
                    max = right;
            }
            if (max == parent)   //父节点元素最大
                return;
            swap(max, parent);
            parent = max;
        }
